...
y_axis_total_particles <- bquote(atop(bold('Intracellular AAV particles'),
                                      "[10" ^{9} ~ "TP.mL" ^{-1} ~"]"))

elisa_plot <- ggplot(total_sum %>% filter(condition=="intra" & time != "24h" & method =="elisa"),aes(x=time, y=mean*10^-9, fill=condition))+
  geom_bar(stat="identity", position=position_dodge(), colour="black", size=.3, fill="grey")+
  geom_errorbar(aes(ymin=mean*10^-9-sd*10^-9, ymax=mean*10^-9+sd*10^-9), width=0.2, position=position_dodge(.9))+
  theme_script() +
  theme(
    legend.title = element_blank(),
  ...
  scale_y_continuous(name=y_axis_total_particles, limits= c(0,80))+
  scale_x_discrete(labels=c("48h" = "48", "72h" = "72","96h" = "96"))

elisa_plot
ggsave(filename= "/total_capsids.tiff", plot= elisa_plot, path = figsdir, device="tiff", width=12.9, height=12.7, unit = "cm", dpi = 600)


# Plot genomic AAV particles
y_axis_genoimic_particles <- bquote(atop(bold('Intracellular genomic AAV'),
                                         bold("particles")~"[10" ^{9} ~ "VG.mL" ^{-1} ~"]"))

qpcr_plot <- ggplot(total_sum %>% filter(condition=="intra" & time != "24h" & method =="qpcr"),aes(x=time, y=mean*10^-9, fill=condition))+
  geom_bar(stat="identity", position=position_dodge(), colour="black", size=.3, fill="grey")+
  geom_errorbar(aes(ymin=mean*10^-9-sd*10^-9, ymax=mean*10^-9+sd*10^-9), width=0.2, position=position_dodge(.9))+
  theme_script() +
  theme(
    ...
    legend.position = "none")+
  xlab(bquote(bold(~"Time after infection") ~ "[hours]"))+
  scale_y_continuous(name=y_axis_genoimic_particles, limits= c(0,80))+
  scale_x_discrete(labels=c("48h" = "48", "72h" = "72","96h" = "96"))

qpcr_plot

ggsave(filename= "/genomic_capsids.tiff", plot= qpcr_plot, path = figsdir, device="tiff", width=12.9, height=12.7, unit = "cm", dpi = 600)
