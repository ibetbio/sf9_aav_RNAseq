...
#===============================================================================
#This script allows the reproduction of figure 4 of the manuscript
...
# Make euclidean sample distance matrix (not used in manuscript)
...
pcaData_overall$condition <- factor(pcaData_overall$condition, levels = c("seed_0hpi", "non_24hpi", "non_48hpi", "inf_24hpi", "inf_48hpi"))
### Make PCA plot
pca_plot_overall <- ggplot(pcaData_overall, aes(x = PC1, y = PC2, color = condition)) +
  ...
  scale_color_manual(labels=c("inf_24hpi"="Infected - 24 hpi", "inf_48hpi"="Infected - 48 hpi", "non_24hpi" = "Non-infected - 24 hpi", "non_48hpi" = "Non-infected - 48 hpi", "seed_0hpi" = "Pre-infection - 0 hpi"), values = c("#4DAF4A" , "#84D5DE", "#1868AA", "#F04949", "#B30C0C"))+
  theme(legend.position ="bottom",
        legend.title = element_blank(),
        ...
        aspect.ratio = 0.5)+
  guides(color=guide_legend(nrow=2, byrow=TRUE, override.aes=list(shape=c(18, 16, 15, 16, 15 ))))
...
volcano_infvsnon24 <- ggplot(data=DE_event24) +
  ...
  ggtitle("Inf.24hpi vs Non-Inf.24hpi")+
  geom_hline(
...
volcano_inf_48vs24 <- ggplot(data=DE_event48) +
  ...
  theme(legend.position = "none", legend.title = element_blank()) +
  xlab(bquote(bold(~log[2] ~ "fold change"))) + ylab(bquote(bold(~-log[10] ~ "BH adjusted p-value")))+
  ggtitle(("Inf.48hpi vs Inf.24hpi"))+
  geom_hline(
    yintercept = -log10(0.05),
    linetype = "dashed",
    color = "gray"
...
### Get common DE genes between 2 comparisons
load(paste(intermdir, "/deres_inf48vs24.rData", sep=""))
load(paste(intermdir, "/deres_infvsnon24.rData", sep=""))

length(intersect(row.names(deres_infvsnon24), row.names(deres_inf48vs24)))
